...
while(<>) {
    ...
    while (/(\d+)v\s*(\d+)Hz/g) {
        ...
        if ($f==$lastf[0]) {          $newf[0]= $f;
        } elsif ($f==$lastf[1]) {     $newf[1]= $f;
        } elsif ($f==$lastf[1]) {     $newf[2]= $f;
        } else {
          my $x= 0;
          while($x<3) {
    ...
    if ($n) {
        $abc= "";
        $p= !$newf[0]? 0: 1000000/16/$newf[0]; $abc.= sprintf("\\x%02x\\x%02x", $p&255, $p/256);
        $p= !$newf[1]? 0: 1000000/16/$newf[1]; $abc.= sprintf("\\x%02x\\x%02x", $p&255, $p/256);
        $p= !$newf[2]? 0: 1000000/16/$newf[2]; $abc.= sprintf("\\x%02x\\x%02x", $p&255, $p/256);
